<div wire:poll.5s class="space-y-6 sm:space-y-8" data-pass-id="{{ $mobilePass->id }}">
    ...
    @if (! $installed)
        <x-pass-state heading="Scan to install on your iPhone">
            <x-slot:visual>
    ...
    @elseif (! $hasChanged)
        <x-pass-state heading="Now push a live update">
            <x-slot:visual>
            ...
            <x-slot:pill>Installed on your iPhone</x-slot:pill>
    ...
    @else
        <x-pass-state heading="Apple has been notified">
            <x-slot:visual>
            ...
            <x-slot:pill>Update queued</x-slot:pill>
